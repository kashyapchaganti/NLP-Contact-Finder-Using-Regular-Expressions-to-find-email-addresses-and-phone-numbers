...
epatterns.append('(\b\w+\b)\s*(?:at|@)?\s*cs\.?\s*(?:dot|\.)?\s*(?:stanford|\.)?\s*(?:dot|\.)?\s*(?:edu)?')
...
epatterns.append('([A-Za-z0-9.]+)\\s*[<][A-Za-z0-9.]+[>]\\s*@\\s*([A-Za-z0-9._]+)\\.edu')
epatterns.append('([A-Za-z0-9._]+)\\s*<at[\\s*A-Za-z0-9\\s*]+>\\s*([A-Za-z0-9._]+)\\.edu')
epatterns.append('([A-Za-z0-9._]+)\\s*at\\s*[<][A-Za-z0-9.!\\s_-]+[>]\\s*([A-Za-z0-9._]+)\\s*[<][A-Za-z0-9.!\\s_-]+[>]\\s*dot\\s*[<][A-Za-z0-9.!\\s_-]+[>]\\s*edu')
...
extrapatterns=[]
extrapatterns.append('([A-Za-z0-9.]+)\s*at\s*([A-Za-z0-9]{2,})\s*dot\s*([A-Za-z0-9_]{2,})\s*dot\s*edu')
extrapatterns.append('([A-Za-z0-9.]+)\s*at\s*([A-Za-z0-9]{2,})\s*([A-Za-z0-9_]{2,})\s*edu')
extrapatterns.append('([A-Za-z0-9.]+)\s*at\s*([A-Za-z0-9]{2,})\s*;\s*([A-Za-z0-9_]{2,})\s*;\s*edu')
...
def process_file(name, f):
    ...
    for line in f:
        ...
        # Removing everything between and including "(followed by" and replacing it with '@'

        if '-@-' in line:
            line = re.sub('-', '', line)
            # Removing all occurrences of '-' if the line contains '-@-' pattern

        line = re.sub('\s+(?:DOT|dot|DOM)\s+', '.', line)
        line = re.sub('\s+(?:AT|WHERE)\s+', '@', line)
        # Replacing occurrences of 'DOT', 'dot', 'DOM' with '.' and 'AT', 'WHERE' with '@'
        line = re.sub('&#x40;', '@', line)
        ...
        for epat in epatterns:
            ...
            if ('com' in epat or 'COM' in epat):                    #for displaying com instead of edu in case of epattern containing com domain instead of edu
                for m in matches:
            ...
            elif ('obfuscate' in epat):                               #for changing order of userid and domain part in obfuscate epattern
                for m in matches:
                    email=m[1]+'@'+m[0]+'.edu'
                    res.append((name,'e',email))
            else:
        ...
        for newpat in extrapatterns:
            # each newpat has 3 sets of parentheses so each match will have 3 items in a list
